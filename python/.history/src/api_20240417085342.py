    ...
    try:
        ...
        for file in os.listdir(new_folder_path + "/output_img_pt/0/cls"):
            if file.endswith(".txt"):
                with open(os.path.join(new_folder_path + "/output_img_pt/0/cls", file), "r") as f:
                    # 将单一文件中的所有结果分成二级字典
                    for line in f.readlines():
                        cls_name, score = line.strip().split(" ")
                        if cls_name not in cls_result:
                            cls_result[cls_name] = []
                        cls_result[cls_name].append(float(score))
