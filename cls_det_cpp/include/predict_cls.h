...
void pre_process(cv::Mat &image, cv::Mat &blob) {
  ...
  cv::dnn::blobFromImage(crop_image, blob, 1,
                         cv::Size(crop_image.cols, crop_image.rows),
...
// 后处理
const std::string &post_process(std::vector<cv::Mat> &detections,
                                std::vector<std::string> &class_name) {
  std::vector<float> values;
  for (size_t i = 0; i < detections[0].cols; i++) {
    values.push_back(detections[0].at<float>(0, i));
  }
  int id = std::distance(values.begin(),
...
void cls() {
  ...
  // 加载预训练的神经网络模型
  cv::dnn::Net net = cv::dnn::readNet("../best_cls.onnx");
  // 设置网络的计算后端和目标为CUDA
  net.setPreferableBackend(cv::dnn::DNN_BACKEND_CUDA);
  net.setPreferableTarget(cv::dnn::DNN_TARGET_CUDA);

  // 遍历输入图片目录，对每张图片进行预处理和网络推理，然后将结果写入文件
  std::filesystem::path path("../output_text_img");
  std::fstream out_file("../output_cls.txt", std::ios::out);
  std::vector<cv::Mat> outputs;

  for (const auto &entry : std::filesystem::directory_iterator(path)) {
    cv::Mat image = cv::imread(entry.path().string()), blob;
